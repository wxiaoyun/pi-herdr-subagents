...
const err = (t: string) => ({ content: [{ type: "text" as const, text: t }], details: {}, isError: true });
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		promptSnippet: "Spawn a child pi agent (profiles: general-purpose, Worker, Scout, plus user profiles)",
		promptGuidelines: [
		...
		parameters: Type.Object({
			prompt: Type.String({ description: "Task for the child. Self-contained, the child has no conversation context." }),
			description: Type.String({ description: "3-5 word summary shown in listings." }),
			subagent_type: Type.Optional(Type.String({ description: "Profile name. Default general-purpose." })),
			model: Type.Optional(Type.String({ description: "provider/model[:thinking]. Default inherits the parent model." })),
			thinking: Type.Optional(Type.String({ description: "off|minimal|low|medium|high|xhigh|max" })),
			cwd: Type.Optional(Type.String({ description: "Working directory. Default parent cwd." })),
			run_in_background: Type.Optional(Type.Boolean({ description: "Default false." })),
			name: Type.Optional(Type.String({ description: "Short handle used in the agent id." })),
			resume: Type.Optional(Type.String({ description: "Existing agent id to continue with this prompt." })),
			timeout_ms: Type.Optional(Type.Number({ description: "0 = no timeout. On timeout returns partial output, child keeps running." })),
		}),
		async execute(_id, p, signal, _onUpdate, ctx) {
			...
			if (!profile) return err(`unknown subagent_type ${typeName}. Available: ${[...profiles.keys()].join(", ")}`);
			if (myProfile) {
				...
				const r = await m.spawn(
					{
						...
						profile,
						model: p.model ?? profile.model ?? settings.defaultModel ?? (ctx.model ? `${ctx.model.provider}/${ctx.model.id}` : undefined),
	...
	pi.registerTool({
		...
		description: "Status and output of a child agent. With wait=true blocks until it finishes or blocks on a question.",
		parameters: Type.Object({
		...
		async execute(_id, p, signal, _u, ctx) {
			try {
				const settings = loadSettings(ctx.cwd);
				return text(await getManager(ctx.cwd).result(p.agent_id, p.wait ?? false, p.timeout_ms ?? settings.defaultTimeoutMs, signal));
			} catch (e) {
				return err(String(e));
			}
		},
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			to: Type.Optional(Type.String({ description: "Agent id or pane id. Default: parent." })),
			message: Type.String(),
			kind: Type.Optional(StringEnum(["message", "interrupt", "keys"] as const)),
			expect_reply: Type.Optional(Type.Boolean({ description: "Mark this agent as waiting for the parent." })),
		}),
		async execute(_id, p, _s, _u, ctx) {
			const to = p.to ?? parentPane;
			if (!to) return err("no `to` given and this agent has no parent");
			const prefix = !p.to && process.env.PI_HERDR_SUBAGENT_ID ? `[from ${process.env.PI_HERDR_SUBAGENT_ID}] ` : "";
			try {
				await getManager(ctx.cwd).send(to, prefix + p.message, p.kind ?? "message");
			} catch (e) {
				return err(`send_message failed: ${String(e)}`);
			}
			if (p.expect_reply && parentPane && !p.to) {
				awaitingParent = true;
				pi.events.emit("herdr:blocked", { active: true, label: "awaiting parent" });
				return text("Sent to parent. End your turn now and wait for the reply.");
			}
	...
	pi.registerCommand("agents", {
		...
		handler: async (_args, ctx) => {
			...
			const labels = kids.map((c) => `${c.id}  ${c.status.padEnd(8)} ${c.profile.padEnd(16)} ${c.pane ?? "-"}  ${c.description}`);
			...
			if (action === "focus") await m.focus(child.id).catch((e) => ctx.ui.notify(String(e), "error"));
			if (action === "kill") await m.kill(child.id).catch((e) => ctx.ui.notify(String(e), "error"));
		},
	});
}
