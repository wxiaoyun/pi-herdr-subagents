...
export type ChildStatus = "queued" | "starting" | "running" | "blocked" | "done" | "timeout" | "killed";
...
const slug = (s: string): string => s.toLowerCase().replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "") || "agent";
...
export class Manager {
	...
	private piArgs(o: SpawnOpts, id: string, session?: string): string[] {
		...
		if (p.systemPrompt) args.push(p.promptMode === "replace" ? "--system-prompt" : "--append-system-prompt", p.systemPrompt);
		if (p.tools?.length) {
			const spawnTools = p.allowedSubagents === "all" || p.allowedSubagents.length ? SPAWN_TOOLS : [];
	...
	private async launch(child: Child, o: SpawnOpts, session?: string): Promise<void> {
		...
		try {
			await this.h.agentStart(child.id, child.pane, this.piArgs(o, child.id, session));
		} catch (e) {
		...
		log("launched", { id: child.id, pane: child.pane, session: child.sessionPath });
	}
	...
	async spawn(o: SpawnOpts, signal?: AbortSignal): Promise<SpawnResult> {
		...
		if (o.background) {
			if (this.live >= this.settings.maxConcurrent) {
				void this.acquire().then(run).catch((e) => this.fail(child, e));
				return { id: child.id, status: "queued", text: `${child.id} queued (${this.live}/${this.settings.maxConcurrent} slots busy)` };
			}
			await this.acquire();
			...
			this.watchPrompt(child, o.prompt, o.timeoutMs);
			return { id: child.id, status: "running", text: `${child.id} started in pane ${child.pane}` };
		}
	...
	private async resume(o: SpawnOpts, signal?: AbortSignal): Promise<SpawnResult> {
		const child = this.children.get(o.resume!);
		if (!child) throw new Error(`unknown subagent ${o.resume}`);
		child.background = o.background;
		const alive = await this.h.agentGet(child.id).then(() => true, () => false);
		if (!alive) {
			if (!child.sessionPath) throw new Error(`${child.id} is gone and has no session to resume`);
			await this.acquire(signal);
	...
	private async foreground(child: Child, prompt: string | undefined, timeoutMs: number, signal?: AbortSignal): Promise<SpawnResult> {
		try {
			const info = prompt
			...
			if (signal?.aborted) {
				...
				return { id: child.id, status: "detached", text: `${child.id} detached, keeps running in pane ${child.pane}` };
			}
	...
	private watchPrompt(child: Child, prompt: string, timeoutMs: number): void {
		this.spawnWatcher(child, (signal) => this.h.agentPromptWait(child.id, prompt, timeoutMs, signal), timeoutMs);
	}

	private watch(child: Child, timeoutMs: number): void {
		this.spawnWatcher(child, (signal) => this.h.agentWait(child.id, timeoutMs, signal), timeoutMs);
	}

	private spawnWatcher(child: Child, op: (signal: AbortSignal) => Promise<AgentInfo>, timeoutMs: number): void {
		child.watcher?.abort();
		...
		void op(ac.signal)
			...
			.catch(async (e) => {
				...
				if (e instanceof HerdrError && e.code === "timeout") await this.timeout(child);
	...
	private async finish(child: Child, info: AgentInfo): Promise<void> {
		...
		if (this.settings.closeOnDone && child.pane) {
			await this.h.paneClose(child.pane).catch((e) => log("pane_close", { id: child.id, error: String(e) }));
		}
	}
	...
	private fail(child: Child, e: unknown): void {
		...
		child.status = "killed";
		child.report = { text: `failed: ${String(e)}`, usage: { input: 0, output: 0, cost: 0, turns: 0 } };
		this.release(child);
	}
	...
	private async collect(child: Child): Promise<Report> {
		...
		return { text: screen.trim(), usage: { input: 0, output: 0, cost: 0, turns: 0 } };
	}
	...
	private deliver(child: Child): void {
		...
		if (this.settings.notify === "followUp") {
			...
			this.pi.sendMessage({ customType: "herdr-subagent", content: text, display: true }, { deliverAs: "nextTurn" });
		}
	}

	// ---- inspect / message / kill ---------------------------------------------

	async result(id: string, wait: boolean, timeoutMs: number, signal?: AbortSignal): Promise<string> {
		...
		if (child.status === "done" || child.status === "killed") return this.formatReport(child);
		const recent = await this.h.agentRead(id, 40).catch(() => "");
		return `[subagent ${id} | ${child.profile} | ${child.status} | pane ${child.pane}]\n${recent.trim()}`;
	}

	async send(to: string, message: string, kind: "message" | "interrupt" | "keys"): Promise<void> {
		if (kind === "keys") {
		...
		try {
			...
			const pane = this.children.get(to)?.pane ?? (await this.h.agentGet(to)).pane;
	...
	async kill(id: string): Promise<void> {
		...
		if (!child) throw new Error(`unknown subagent ${id}`);
		child.watcher?.abort();
		if (child.pane) await this.h.paneClose(child.pane).catch((e) => log("kill_close", { id, error: String(e) }));
		child.status = "killed";
		this.release(child);
	}
